			...
			<div class="col-md-11">
				...
				<table class="table table-responsive table-bordered" id="my-table">
					...
					@foreach($mails as $key => $mail )
						<tr  data-toggle="modal" data-target="#myModal{{$mail->id}}" class="clickable" id="{{$mail->id}}"  data-backdrop="static" data-keyboard="false">
							<td>{{($key+1)}}</td>
							<td>{{$mail->subject}}</td>
		  ...
		  <div class="modal-dialog" role="document">
		    <div class="modal-content">
		      ...
		      <div class="modal-footer">
		        ...
		        <a href="{{url('read')}}/{{$mail->id}}" class="btn btn-danger" >Close</a>
		      </div>
		    </div>
		  </div>
		</div>
...
<script>
	$(document).ready(function(){
		/*$('.myModal').on('hide.bs.modal',function(e){
			e.preventDefault();
			var id = $(this).attr('id');
			window.location.href = "{{url('read')}}/"+id;
		});*/
		$('#my-table').DataTable();
	});
</script>
